...
option_screen:
	...
	os_init_msg		db 'ShoockOS', 0
	os_version_msg		db 'Wersja ', SHOOCKOS_VER, 0

	dialog_string_1		db 'ShoockOS v', SHOOCKOS_VER, 0
	dialog_string_2		db '', 0
	dialog_string_3		db 'Zycze milego uzytkowania!', 0
...
app_selector:
	...
	jne invalid_extension		; If not, show an error message
...
invalid_extension:
	pop si				; We pushed during the extension check
	mov ax, ext_string_1
	mov bx, ext_string_2
	mov cx, 0
	mov dx, 0			; One button for dialog box
	...
	kerndlg_string_1	db 'Nie mozna zaladowac pliku KERNEL.BIN', 0
	kerndlg_string_2	db 'KERNEL.BIN nie jest', 0
	kerndlg_string_3	db 'normalnym programem!', 0

	ext_string_1		db 'Nie mozna uruchomic pliku', 0
	ext_string_2		db 'o roszerzeniu innym niz .bin', 0
